...
const Productfulldetails = () => {
    ...
    const deleteProduct = async (id) => {
        ...
        if (isConfirmed) {
        try {
        const res = await axios.delete(`http://localhost:3005/snitch/deleteproduct/${id}`);
...
<div className='body10'>
      <div className="card5">
  <nav>
  <div>  <svg className="arrow" version="1.1" viewBox="0 0 512 512" width="512px"  xmlns="http://www.w3.org/2000/svg" ><polygon points="352,115.4 331.3,96 160,256 331.3,416 352,396.7 201.5,256 " stroke="#727272"/></svg>
...
{/*    
  ...
  <div className="carousel-inner img2">
  ...
  <div className="description">
   ...
   <div className='stocksub'>
    ...
    <div className='stocksub'>
        <div>: {getProducts?.stock?.size_S}</div>
        <div>: {getProducts?.stock?.size_M}</div>
        <div>: {getProducts?.stock?.size_L}</div>
        <div>: {getProducts?.stock?.size_XL}</div>
